...
printf("\nSTEPENABLE\n");
memory(STEPENABLE,'w',0x8,'o');         // Habilita o STEP1 e STEP3
}
/********************************************/
...
void imprime(void)
{
unsigned int i = 0, resultado = 0, stepID = 0, FSM = 0, old = 0, new = 0, flag = 0;
float tensao = 0.0;
for(i=0;i<3;i++)
    {
    // Aguarda uma nova conversão
    while(((memory(ADCSTAT,'r',0x0,'o'))&(0x20))==0) printf("\nIDLE...");    // Aguarda a FSM sair do estado IDLE
    old = (memory(FIFO1COUNT,'r',0x0,'o'))&(0x7F);                           // Aguarda uma nova palavra ser escrita no FIFO (incremento em FIFOCOUNT)
    new = old;
    printf("\n\nFIFO1COUNT: %d",new);
    do
        {
        if(flag==0) printf("\nConvertendo...");
        flag++;
        new = (memory(FIFO1COUNT,'r',0x0,'o'))&(0x7F);
        }
    while(new == old);
    flag = 0;
    printf("\nFIFO1COUNT: %d\n",new);
    ...
    // Validação da medida
    FSM = (memory(ADCSTAT,'r',0x0,'o'))&(0xF);                               // Captura qual step a FSM se encontra
    stepID = ((memory(FIFO1DATA,'r',0x0,'o'))&(0xF0000))>>16;                // Captura step ID da última escrita realizada no FIFO
    if(stepID == FSM) printf("\nMedida válida");                             // Testa se o step ID corresponde ao step da FSM
    else printf("\nMedida inválida");

    // Impressão dos resultados
    resultado = (memory(FIFO1DATA,'r',0x0,'o'))&(0xFFF);                     // Leitura da conversão AD propriamente dita
    tensao = (((float)(resultado))/(4096))*1.8;                              // Regra de três para retorna ao domínio da tensão
    printf("\nStep ID: %d, AIN%d",(stepID+1),stepID);
    printf("\nTensão:  %.3f V",tensao);
    }
printf("\n");
}
